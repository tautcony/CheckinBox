...
COOKIE = os.environ.get("COOKIE_BILIBI")
...
class BilibiliCheckIn(CheckIn):
    def _checkin(self, session, get, post, info, error):  # noqa: C901
        ...
        # 漫画签到
        manga_msg = self.manga_sign(session=session)
        info(manga_msg)
        # 直播签到
        ...
        # 今日已投币数量
        coins_av_count = reward_ret.get("data", {}).get("coins_av", 0) // 10
        # 每日投币经验为50
        coin_remain = 5 - coins_av_count
        ...
        summary_msg = (
            f"今日获得经验: {today_exp}当前经验: {new_current_exp}\n"
