...
def notify(title, *args, **kwargs):
    ...
    if pushplus_token:
        ...
        r = requests.post("http://www.pushplus.plus/send", params=params)
        if r.json()["code"] != 200:
            print(r.json())
    ...
    if CPkey:
        ...
        if content:
            title = title + "\n" + content
        r = requests.post(f"https://push.xuthus.cc/{Smode}/{Skey}", params={
            "c": title
        })
        logging.debug(r.text)
    if tg_token and tg_chatid:
        tg_api_host = tg_api_host if tg_api_host else "api.telegram.org"
        r = requests.post(f"https://{tg_api_host}/bot{tg_token}/sendMessage", data={
            "chat_id": tg_chatid,
            "text": f"{title}\n{content}"
        })
        logging.debug(r.text)
